...
export default function SettingsPage() {
  ...
  return (
    <div className="min-h-screen bg-[#faf9f6] px-6 py-12 text-[#2a2018]">
      <main className="mx-auto w-full max-w-[980px]">
        <div className="mb-8">
          <div className="mb-7 text-[12px] font-bold uppercase tracking-[0.24em] text-[#a58b6c]">Extension & Privacy</div>
          <h1 className="rh-display text-[44px] font-semibold leading-none tracking-[-0.03em] text-[#23170f]">Settings</h1>
        </div>

        <section className="mb-8 flex items-center justify-between rounded-[20px] border border-[#b8c9b5] bg-[linear-gradient(90deg,#edf5ed,#fbfaf6)] px-6 py-5 shadow-[0_8px_30px_rgba(70,45,20,.04)]">
          <div className="flex items-center gap-4">
            <div className="relative grid h-12 w-12 place-items-center rounded-[12px] bg-white shadow-[0_1px_4px_rgba(70,45,20,.12)]">
              ...
              <span className="absolute right-[-3px] top-[-3px] h-4 w-4 rounded-full border-2 border-[#edf5ed] bg-[#6a9a66]" />
            </div>
            <div>
              <h2 className="rh-display text-[22px] font-semibold leading-none text-[#315a32]">Extension plugged in</h2>
              <p className="mt-1 text-[14px] text-[#49674a]">Chrome · v2.4.1 · capturing this session</p>
            </div>
          </div>
          <div className="hidden items-center gap-2 rounded-full bg-[#dfeadd] px-4 py-2 text-[13px] font-semibold text-[#315a32] sm:flex">
        ...
        <section className="mt-8">
          ...
          <div className="grid gap-4 sm:grid-cols-3">
            ...
            <button onClick={resetFresh} className="rounded-[16px] border border-[#e5b8ad] bg-[#fff8f5] p-5 text-left transition hover:-translate-y-0.5 hover:shadow-[0_8px_24px_rgba(70,45,20,.08)]">
        ...
        <div className="mt-10 text-[13px] text-[#9a8266]">
...
function SettingsGroup({ title, rows }: { title: string; rows: readonly (readonly [string, string, boolean, string?])[] }) {
  return (
    <section className="mt-8">
      ...
      <div className="overflow-hidden rounded-[18px] border border-[#e0d2bd] bg-[#f8f1e6] shadow-[0_8px_24px_rgba(70,45,20,.04)]">
        {rows.map(([name, body, enabled, tone], i) => (
          <div key={name} className={`flex items-center justify-between gap-5 px-6 py-4 ${i ? "border-t border-[#e5dac9]" : ""}`}>
            <div>
              <h3 className="text-[16px] font-bold leading-tight text-[#2a2018]">{name}</h3>
              <p className="mt-1 text-[14px] leading-snug text-[#705f4d]">{body}</p>
            </div>
...
function SectionLabel({ children }: { children: ReactNode }) {
  return <div className="mb-3 text-[12px] font-bold uppercase tracking-[0.24em] text-[#a58b6c]">{children}</div>;
}
...
function ActionCard({ title, body }: { title: string; body: string }) {
  return (
    <button className="rounded-[16px] border border-[#e0d2bd] bg-[#f8f1e6] p-5 text-left transition hover:-translate-y-0.5 hover:shadow-[0_8px_24px_rgba(70,45,20,.08)]">
