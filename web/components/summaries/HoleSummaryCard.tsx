...
export function HoleSummaryCard({ hole, publicMode = false }: { hole: RabbitHole; publicMode?: boolean }) {
  ...
  return (
    <section className="overflow-hidden rounded-[24px] border border-[#785a3224] bg-[#fbf6ec] shadow-[0_18px_50px_rgba(70,45,20,.10)]">
      <div className="relative p-7 sm:p-8">
        <div className="absolute right-0 top-0 h-40 w-40 rounded-full blur-3xl" style={{ background: `${accent.hex}24` }} />
        <div className="relative">
          <div className="mb-4 inline-flex items-center gap-2 rounded-full border border-[#785a3224] bg-[#f6efe1] px-3 py-1.5 text-[12px] font-semibold uppercase tracking-[0.16em] text-[#a8967d]">
            <span className="h-2 w-2 rounded-full" style={{ background: accent.hex }} />
            {publicMode ? "Shared rabbit hole" : "End-of-hole summary"}
          </div>
          <h2 className="rh-display max-w-[12ch] text-[44px] font-semibold leading-none text-[#2a2018]">{hole.title}</h2>
          <p className="mt-4 max-w-2xl text-[18px] italic leading-7 text-[#6a5a48]">{question}</p>
...
function Metric({ label, value, sub }: { label: string; value: string; sub: string }) {
  return (
    <div className="rounded-[18px] border border-[#785a321f] bg-[#f6efe1] p-4">
      <div className="text-[11px] font-semibold uppercase tracking-[0.16em] text-[#a8967d]">{label}</div>
      <div className="mt-2 rh-display text-[34px] font-semibold leading-none text-[#2a2018]">{value}</div>
      <div className="mt-1 text-[13px] text-[#8a7860]">{sub}</div>
    </div>
  );
}
...
function Block({ title, items }: { title: string; items: string[] }) {
  return (
    <div className="rounded-[18px] border border-[#785a321f] bg-white/45 p-5">
      <div className="mb-3 text-[12px] font-semibold uppercase tracking-[0.16em] text-[#a8967d]">{title}</div>
      <div className="flex flex-wrap gap-2">
        {items.slice(0, 6).map((item) => (
          <span key={item} className="rounded-full bg-[#f2e9d6] px-3 py-1.5 text-[13.5px] font-semibold text-[#5a4a38]">{item}</span>
        ))}
      </div>
    </div>
  );
